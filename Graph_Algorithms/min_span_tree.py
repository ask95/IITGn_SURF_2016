...
class Edge():
  def __init__(self,startv, stopv, weight, directed_bool):
    self.start_vertice = startv
    self.stop_vertice = stopv
    self.weight = weight
    self.is_directed = directed_bool
...
def present_vertice(i_dic, source_v, search_v):
    present = 0
    for i in i_dic[source_v]:
        if (i[0] == search_v):
            present = 1
            break
    return present
...
def update(input_tuple, dic):
    ...
    for i in input_lst:
        ...
        if start_v not in dic:
            #print "Hi"
            dic.update({start_v:[[stop_v, i.weight]})
            modified_dic = copy.deepcopy(dic)
        else:
            #if stop_v not in dic[start_v]:
            if not present_vertice(dic, start_v, stop_v):
                #print "Hi"
                #print dic
                modified_dic = copy.deepcopy(dic)
        ...
        if stop_v not in dic:
            ...
            if not i.is_directed:
                dic[stop_v].append(start_v)
                modified_dic[stop_v].append(start_v)
...
class findConnectivity():
    ...
    def query_min_spanning_tree(self):
